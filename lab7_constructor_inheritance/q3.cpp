...
public:
    int activity[2];
    sport(int *activ, char *n, int rol, int ag) : student(n, rol, ag)
    {
        for (int i = 0; i < 3; i++)
        {
            activity[i] = *activ++;
...
public:
    result(int *mark, int *activ, char *n, int rol, int ag) : test(mark, n, rol, ag) , sport(activ, n, rol, ag),student(n,rol,ag)
    {
    }
    void gradecal()
    {
...
int main()
{
    ...
    result t(marks_025, activ_marks, name_025, roll_025, age_025);
